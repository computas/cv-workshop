...
1. Fra `frontend`-mappen: kopier **.env.example** til `.env` med `cp .env.example .env`
2. Fyll inn følgende verdier i `.env`:
   1. **VITE_BACKEND_API_URL** = `http://localhost:5007` (sjekk at dette er samme port som backenden kjører på)
   2. **VITE_BACKEND_API_KEY** = api-nøkkelen du genererte i backenden
3. Kjør `npm install`
4. Kjør `npm run dev`
...
Våre erfaringer kommer ferdig sortert fra databasen, men gjerne prøv å bytte rekkefølgen på sorteringen ved å endre rekkefølgen på `a` og `b`.
...
1. Som dere kanskje ser, så funker ikke filtrering akkurat nå. Det er fordi staten `selectedExperienceType` aldri blir oppdatert. cxSelect-komponenten har en `handleSelectChange` funksjon som console.loger valgte type. Oppdater funksjonen `handleSelectChange` så staten endres når man velger type i selecten.
2. Fjern kommenteringen av filteredExperiences() og endre mappingen av erfaringen slik at den bruker filteredExperiences() istedenfor experiences.
<details>
<summary>Vis løsning</summary>
...
Vi skal nå håndtere situasjoner der enkelte erfaringer mangler noen felt. Ikke alle erfaringer er knyttet til en bedrift, et hobbyprosjekt kan for eksempel ha `company: null` fra API-et.

1. Løs oppgaven i `ExperienceCard.tsx`. Sjekk om erfaringen har en `company` property. Hvis ikke, så vis **«Selvstendig arbeid»** i stedet.
...
Det andre eksempelet bruker ternary-operatoren: `betingelse ? verdiHvisTrue : verdiHvisFalse`.
...
1. Lag en modal som viser en erfaring i detalj når man klikker på et erfaringskort. Velg selv hva som er nyttig å vise, det finnes mer data i API-et enn det som vises på kortet i dag, for eksempel inneholde **description**. Kanskje man også kan legge til et ekstra felt i backenden og databasen som man ønsker å vise i modalen, for eksempel en link til hobbyprosjektet ditt.

Tips: Ta utangspunkt i `PopupModal.tsx`
